...
float* forward(
  ...
  for (int l = 0; l < layer_count; l++) {
    ...
    STOP_2(st2, MATMUL_OUTPUT_ATTENTION, pos);
    ...
    matmul(
     ...
     STOP_2(st2, MATMUL_OUTPUT_FFN, pos);
...
// ----------------------------------------------------------------------------
// generation loop
void generate_prompt(transformer_t* transformer, Tokenizer* tokenizer,
    Sampler* sampler, int * sequence,int sequence_len,int* next, size_t* past)
{
  int* chunk = sequence;
  ...
  while (chunk_len != 0) {
    ...
    STOP_1(st2,FORWARD_TIME_PROMPT);
...
void generate_new_token(transformer_t* transformer,
    Tokenizer* tokenizer,
    Sampler* sampler,
    int steps,int past, int next, size_t* generated_count)
{
  double st = 0;
 ...
 */
  ...
  float* warmup = driver(transformer, 1, sequence, 1, 1); // cache warmup

  size_t generated_count = 1; // number of tokens generated so far
  ...
  generate_prompt( transformer,  tokenizer,sampler,  sequence,sequence_len,&next,  &past);
  ...
  start = time_in_ms();
  generate_new_token(transformer,tokenizer,sampler,steps,past,next, &generated_count);
  decode_time = time_in_ms()-start;
  printf("\n");
  // report achieved tok/s
  /* if (past > 2) {
    fprintf(stderr, "Prefill: %f tok/s\n", sequence_len / prefill_time);
    fprintf(stderr, "Decode:  %f tok/s\n", (generated_count - 1) * 1000 / decode_time);
  } */
...
/*   fprintf(stderr, "transformer_t configuration:\n");
  ...
  LET_TAB_INSTR(N_InstrStop1ID_VALUES);
  LET_TAB_INSTR_2(N_InstrStop2ID_VALUES,45);
  // run!
  generate(&transformer, &tokenizer, &sampler, prompt, steps);

  PRINT_JSON_1(string_values_1,N_InstrStop1ID_VALUES);

  PRINT_JSON_2(string_values_2,N_InstrStop2ID_VALUES,  45);
  printf("}");
